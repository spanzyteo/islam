...
import { emergencyAppeal } from '@/app/(main)/data/OneOffData'
...
const EmergencyAppeal = () => {
  ...
  const filteredAppeal = emergencyAppeal.filter(
  ...
  const handleAmountClick = (amount: number) => {
    setIsOther(false)
    dispatch({
      type: 'SET_EMERGENCY_AMOUNT', // Dispatch the predefined amount to the global state
      payload: amount,
    })
  }
  return (
        ...
        {filteredAppeal.map((item) => (
          <>
            ...
            >
              ...
              <button
                onClick={() => {
                  handleAmountClick(180)
                }}
                className={`p-4 px-10 font-bold text-4xl hover:bg-[#01aef0] hover:text-white w-full md:w-auto ${
                  state.emergencyAmount === 180
                    ? 'bg-[#01aef0] text-white'
                    : 'bg-white text-[#555555]'
                }`}
              >
              ...
              <button
                onClick={() => {
                  handleAmountClick(95)
                }}
                className={`p-4 px-10 font-bold text-4xl hover:bg-[#01aef0] hover:text-white w-full md:w-auto ${
                  state.emergencyAmount === 95
                    ? 'bg-[#01aef0] text-white'
                    : 'bg-white text-[#555555]'
                }`}
              >
